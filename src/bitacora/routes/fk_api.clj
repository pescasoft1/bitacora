(ns bitacora.routes.fk-api
  (:require
   [compojure.core :refer [defroutes GET POST]]
   [bitacora.engine.config :as config]
   [bitacora.engine.crud :as crud]
   [bitacora.models.crud :as model-crud]
   [bitacora.models.util :refer [json-response]]
      ...
      (try
        (let [entity-config (config/get-entity-config entity)
              ...
              form-fields (map #(select-keys % [:id :label :type :required? :placeholder
                               ...
                               fields)
              rendered (let [render-fn #'bitacora.engine.render/render-field]
                         (->> fields
                              (map #(render-fn % {}))
                              (html)))]
