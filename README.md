...
This creates `resources/entities/products.edn`, migration files, and `src/bitacora/hooks/products.clj`. Refresh the browser and the entity appears in the menu.
...
```bash
lein uberjar
java -jar target/uberjar/bitacora-07-05-2026-0.1.0-standalone.jar
```
...
{:connections
 {:sqlite   {:db-type  "sqlite"
             ...
             :db-name  "db/bitacora-07-05-2026.sqlite"}

  :mysql    {:db-type  "mysql"
             :db-class "com.mysql.cj.jdbc.Driver"
             :db-name  "//localhost:3306/bitacora-07-05-2026"
             :db-user  "root"
             :db-pwd   "password"}

  :postgres {:db-type  "postgresql"
             :db-class "org.postgresql.Driver"
             :db-name  "//localhost:5432/bitacora-07-05-2026"
             :db-user  "postgres"
             :db-pwd   "password"}
 ...
 :port          3000
 :site-name     "bitacora-07-05-2026"
 :uploads       "./uploads/bitacora-07-05-2026/"
 :max-upload-mb 5
 :theme         "sketchy"}
```
...
```clojure
(require '[bitacora.engine.config :as config])
(config/reload-all!)
```
